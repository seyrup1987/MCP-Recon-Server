...
def _port_scanner(ip: str, start: int, domain: str, timeout: float) -> List[Dict]:
    """Scan a range of ports and return open ones."""
    local_open_ports = []
    for port in tqdm(range(start, min(start + 100, 65536)), desc=f"Ports {start}-{min(start + 99, 65535)}"):
        try:
            soc = st.socket(st.AF_INET, st.SOCK_STREAM)
            st.setdefaulttimeout(timeout)
...
async def portScanner4LLM(ip: str, startPort: int, endPort: int, domain: str, timeout: float):
    """
    Asynchronously scans a range of ports on a given IP/domain.

    Args:
        ip: IP address of the domain to be scanned.
        startPort: Starting port number.
        endPort: Ending port number (inclusive).
        domain: Domain name for context (e.g., SSL hostname).
        timeout: Timeout in seconds for each port connection attempt.

    Returns:
        A list of dictionaries, where each dictionary contains details about an open port.
    """
    local_open_ports = []
    # Ensure endPort is treated correctly in range and doesn't exceed max port
    actual_end_port = min(endPort + 1, 65536) # Include endPort, limit to 65535
    port_ranges = list(range(startPort, actual_end_port))
    random.shuffle(port_ranges) # Keep randomization if desired

    for port in port_ranges:
        service_info = None
        try:
            # Use asyncio's open_connection for non-blocking socket operations
            ...
            # Run detect_service_and_os in a thread to avoid blocking the event loop
            service_info = await asyncio.to_thread(
                detect_service_and_os, ip, port, domain, timeout
            )

            # Check if service_info indicates an error during detection
            if service_info and not service_info.get('error'):
                 with ports_lock: # Still need lock if modifying shared global list
                     open_ports.append(service_info)
                 local_open_ports.append(service_info)
            elif service_info and service_info.get('error'):
                 # Log the error from detect_service_and_os if needed
                 print(f"Error detecting service on port {port}: {service_info['error']}")

        except (asyncio.TimeoutError, ConnectionRefusedError, OSError):
            # Port is likely closed or unreachable
            pass # Ignore ports that are not open
        except Exception as e:
            # Catch other unexpected errors during scanning a single port
...
def port_scanner_parallel(domain: str, start: int, end: int, max_workers: int = 10, timeout: float = 1.0, stealth: bool = True) -> Dict:
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            futures = [executor.submit(_port_scanner, target_ip, port_start, domain, timeout)
...
if __name__ == '__main__':
    ...
    if result.get('Open_Ports'):
        if sslAnalysis:
            ...
            ssl_results = [asyncio.run(analyze_ssl_tls(domain, p['port'])) for p in result['Open_Ports']]
            result['SSL_Analysis'] = ssl_results

        print(f"\nScan results for {domain} ({result['IP_Address']}):")
        for service_info in result['Open_Ports']:
        ...
        if sslAnalysis:  # Fixed typo from 'analysessl' to 'ssl'
            print("\nSSL/TLS Analysis:")
            for i, analysis in enumerate(result['SSL_Analysis']):
