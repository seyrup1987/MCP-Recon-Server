...
def task_factory(loop, coro):
    ...
    app_logger.debug(f"Task created: {task_name} for coroutine: {coro}")
    
    def log_task_done(t):
        try:
            if t.exception():
                app_logger.debug(f"Task finished: {task_name} with exception: {t.exception()}")
            else:
                app_logger.debug(f"Task finished: {task_name} successfully")
        except asyncio.CancelledError:
            app_logger.debug(f"Task cancelled: {task_name}")
        except Exception as e:
            app_logger.error(f"Error logging task completion: {e}", exc_info=True)
...
try:
    ...
    app_logger.info("No running event loop found, will configure when loop starts")
...
async def port_scanner(ip: str, startPort: int, endPort: int, domain: str, timeout: float):
    ...
    try:
        ...
        app_logger.debug(f"[{tool_name}] Calling portScanner4LLM for {target_info} with operation timeout {operation_timeout}s")
        result = await asyncio.wait_for(portScanner4LLM(ip, startPort, endPort, domain, timeout), timeout=operation_timeout)
        app_logger.info(f"Successfully completed {tool_name} for {target_info}")
        return result
    except asyncio.TimeoutError:
        app_logger.error(f"[{tool_name}] Operation timed out after {operation_timeout}s for {target_info}")
        return {"error": f"{tool_name} operation timed out"}
    except Exception as e:
        app_logger.error(f"Error during {tool_name} for {target_info}: {e}", exc_info=True)
        return {"error": str(e)}
...
async def dns_enumerator(domain: str):
    ...
    app_logger.info(f"Starting {tool_name} for {target_info}")
    try:
        # Set a timeout for the operation - increased timeout
        operation_timeout = 180.0
        app_logger.debug(f"[{tool_name}] Calling dnsEnumerator for {target_info} with operation timeout {operation_timeout}s")
...
async def dir_enumerator(domain: str):
    ...
    try:
        # Set a timeout for the operation
        operation_timeout = 300.0
        app_logger.debug(f"[{tool_name}] Calling dirEnum4LLM for {target_info} with operation timeout {operation_timeout}s")
...
async def subdomain_enumerator(domain: str):
    ...
    try:
        ...
        app_logger.debug(f"[{tool_name}] subDomainEnumSync for {target_info} has finished executing in the thread.")
        ...
        app_logger.debug(f"[{tool_name}] Awaiting future result for {target_info}")
        ...
        app_logger.error(f"[{tool_name}] Operation timed out after {operation_timeout}s (or during future wait) for {target_info}")
...
async def web_crawler_proxy_hunter(domain: str):
    ...
    app_logger.info(f"Starting {tool_name} ({target_info})")
    try:
        # Set a timeout for the operation
        operation_timeout = 600.0 # Increased timeout
        app_logger.debug(f"[{tool_name}] Calling webCrawler_proxyHunter with operation timeout {operation_timeout}s")
        ...
        app_logger.info(f"Successfully completed {tool_name}")
        return result
    except asyncio.TimeoutError:
        app_logger.error(f"[{tool_name}] Operation timed out after {operation_timeout}s")
        return {"error": f"{tool_name} operation timed out"}
    except Exception as e:
        app_logger.error(f"Error during {tool_name}: {e}", exc_info=True)
        return {"error": str(e)}
...
async def webSearch(query: str):
    ...
    try:
        ...
        def sync_search():
             app_logger.debug(f"[{tool_name}] Executing synchronous DDGS search for {target_info}")
             ...
             try:
                 with DDGS() as ddgs:
                     ...
                     app_logger.debug(f"[{tool_name}] Synchronous DDGS search completed for {target_info}, found {len(results)} results")
                     return results
             except Exception as thread_err:
                 app_logger.error(f"[{tool_name}] Error inside sync_search thread for {target_info}: {thread_err}", exc_info=True)
                 raise # Re-raise the exception so asyncio.to_thread catches it

        app_logger.debug(f"[{tool_name}] Calling sync_search via asyncio.to_thread for {target_info}")
        ...
        app_logger.info(f"Successfully completed {tool_name} for {target_info}, found {len(results)} results.")
        return {'results': results}
    # No specific TimeoutError expected here unless the thread itself hangs for longer than a global timeout
    except Exception as error:
        app_logger.error(f"Error during {tool_name} for {target_info}: {error}", exc_info=True)
...
async def webSearch4CVEs(technology: str):
    ...
    try:
        def sync_cve_search():
            app_logger.debug(f"[{tool_name}] Executing synchronous DDGS CVE search for {target_info}")
            try:
                with DDGS() as ddgs:
                    # Consider limiting results further if needed
                    results = list(ddgs.text(search_query, max_results=30))
                    app_logger.debug(f"[{tool_name}] Synchronous DDGS CVE search completed for {target_info}, found {len(results)} results")
                    return results
            except Exception as thread_err:
                 app_logger.error(f"[{tool_name}] Error inside sync_cve_search thread for {target_info}: {thread_err}", exc_info=True)
                 raise

        app_logger.debug(f"[{tool_name}] Calling sync_cve_search via asyncio.to_thread for {target_info}")
...
async def ingestText2DB(inputText: str, metadata: str):
    ...
    app_logger.warning(f"Calling deprecated {tool_name} (single chunk) for {target_info}. Consider using process_and_ingest_documentation.")
    try:
        # Assuming ingest2DB from ingest2DB.py is already async
        app_logger.debug(f"[{tool_name}] Calling ingest2DB for {target_info}")
        results = await ingest2DB(inputText, metadata)
        app_logger.info(f"Successfully completed {tool_name} for {target_info}")
        return {"results": str(results)} # Ensure results are stringified if needed
    except Exception as error:
        app_logger.error(f"Error in {tool_name} for {target_info}: {error}", exc_info=True)
        return {'error': str(error)}
...
async def getWebPages(url: str):
    ...
    try:
        ...
        def sync_fetch():
            ...
            app_logger.debug(f"[{tool_name}] Executing synchronous fetch for {target_info} with timeout {fetch_timeout}s")
            try:
                request = urllib.request.Request(url, headers={'User-Agent': USER_AGENT})
                with urllib.request.urlopen(request, timeout=fetch_timeout) as response:
                     content_type = response.headers.get('Content-Type', '').lower()
                     if 'text/html' not in content_type:
                         app_logger.warning(f"[{tool_name}] Content-Type for {url} is '{content_type}', not text/html. Parsing might be suboptimal.")
                     html = response.read()
                     charset = response.headers.get_content_charset() or 'utf-8'
                     app_logger.debug(f"[{tool_name}] Fetched {len(html)} bytes from {url}, status: {response.status}, charset: {charset}")
                     try:
                         return html.decode(charset)
                     except UnicodeDecodeError:
                         app_logger.warning(f"[{tool_name}] Failed to decode {url} with charset {charset}. Falling back to utf-8 with error handling.")
                         return html.decode('utf-8', errors='replace')
            except Exception as thread_err:
                 # Catch errors within urlopen/read/decode
                 app_logger.error(f"[{tool_name}] Error inside sync_fetch thread for {target_info}: {thread_err}", exc_info=True)
                 raise # Re-raise


        app_logger.debug(f"[{tool_name}] Calling sync_fetch via asyncio.to_thread for {target_info}")
        ...
        if not html_content:
             app_logger.warning(f"[{tool_name}] No content fetched from {url}")
             return {'visibleText': ''}

        app_logger.debug(f"[{tool_name}] Parsing HTML content ({len(html_content)} chars) from {url} using BeautifulSoup")
        ...
        app_logger.info(f"Successfully completed {tool_name} for {target_info}, extracted {len(visibleText)} characters.")
        return {'visibleText': visibleText}

    # except asyncio.TimeoutError: # Only if using wait_for around to_thread
    #     app_logger.error(f"[{tool_name}] Overall operation timed out for {target_info}")
    #     return {"error": f"Overall operation timed out"}
        ...
        if isinstance(e.reason, TimeoutError) or 'timed out' in str(e.reason).lower():
             app_logger.error(f"[{tool_name}] URL fetch timed out for {target_info}: {e.reason}")
             return {"error": f"URL fetch timed out: {e.reason}"}
        else:
             app_logger.error(f"[{tool_name}] URL Error fetching {target_info}: {e.reason}")
...
async def process_and_ingest_documentation(documentation_text: str, library_name: str, language: str, source_url: str):
    ...
    if not documentation_text:
        app_logger.warning(f"[{tool_name}] No documentation text provided for {target_info}.")
    ...
    try:
        ...
        app_logger.debug(f"[{tool_name}] Calling chunk_and_ingest for metadata: '{metadata}'")
        ...
        if 'error' in result:
             app_logger.error(f"[{tool_name}] Chunking/Ingestion failed for {target_info}: {result.get('error')}", exc_info=True) # Added exc_info
             # Return the error structure from chunk_and_ingest
             return result
        else:
             app_logger.info(f"Successfully completed {tool_name} for {target_info}. Chunks: {len(result.get('chunk_ids', []))}")
             # Return the success structure from chunk_and_ingest
             return result

    except Exception as e:
        app_logger.error(f"Unexpected error calling {tool_name} for {target_info}: {e}", exc_info=True)
...
# --- Entry Point ---
def entryPointFunction():
    """
    Main entry point for the MCP_Pentest application.
    Initializes the asyncio event loop with enhanced debugging and runs the MCP server.
    """
    app_logger.info(f"Starting MCP_Pentest application via entryPointFunction. Logging to: {LOG_FILE}")
    
    try:
        # Configure asyncio loop with enhanced logging if not already done
        try:
            loop = asyncio.get_running_loop()
        except RuntimeError:
            ...
            loop.set_exception_handler(handle_asyncio_exception)
            loop.set_task_factory(task_factory)
            app_logger.info("Created and configured new asyncio event loop with enhanced logging")
        
        # Use anyio task group for better async management
        app_logger.debug("Starting MCP server...")
        mcpServer.run()
        app_logger.info("MCP server finished running.")
    except AttributeError:
        app_logger.warning("SIGPIPE is not available on this platform (likely Windows).")
    except ValueError:
        app_logger.warning("Cannot set SIGPIPE handler in non-main thread.")
    except (BrokenPipeError, anyio.BrokenResourceError) as e:
        # These errors often occur during normal shutdown when the client disconnects
        app_logger.warning(f"Communication channel closed or broken: {type(e).__name__} - {str(e)}")
        sys.exit(0)  # Treat as graceful exit
    except KeyboardInterrupt:
        app_logger.info("Received KeyboardInterrupt. Shutting down MCP_Pentest application.")
        sys.exit(0)  # Clean exit
    except Exception as error:
        app_logger.critical(f"Critical error running MCP server: {str(error)}", exc_info=True)
        sys.exit(1)  # Exit with error code
    finally:
        app_logger.info("MCP_Pentest application shutting down.")


if __name__ == "__main__":
    app_logger.info("MCP Pentest application starting...")
    # Configure signal handlers if needed
    entryPointFunction()
